...
func TestAppendStagingResumesAtDurableOffset(t *testing.T) {
	...
	if next != 11 {
		t.Fatalf("offset=%d want 11", next)
	}
	...
	if session.Offset != 11 {
		t.Fatalf("durable offset=%d want 11", session.Offset)
	}
}
...
func TestAppendStagingRejectsOffsetMismatch(t *testing.T) {
	...
	space := "11111111-1111-1111-1111-111111111111"
	upload := "22222222-2222-2222-2222-222222222222"
	if _, err := store.AppendStaging(space, upload, 0, strings.NewReader("abc"), 8); err != nil {
		t.Fatal(err)
	}
	...
	if durable != 3 {
		t.Fatalf("durable offset=%d want 3", durable)
	}
}
...
func TestAppendStagingRollsBackOversizedChunk(t *testing.T) {
	...
	space := "11111111-1111-1111-1111-111111111111"
	upload := "22222222-2222-2222-2222-222222222222"
	if _, err := store.AppendStaging(space, upload, 0, strings.NewReader("toolong"), 3); err == nil {
